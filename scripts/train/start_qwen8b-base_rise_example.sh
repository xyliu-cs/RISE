...
WANDB_TOKEN=xxxx
RUN_NAME=xxxx
DATA_DIR=/path/to/deepmath_10K
MODEL_DIR=/path/to/Qwen3-8B-Base
SAVE_DIR=/path/to/Qwen3-8B-Base-DeepMath10K-PPO-RISE

mkdir -p .checkpoints/$RUN_NAME
mkdir -p $SAVE_DIR

# set http_proxy if needed
ray start --head --num-cpus=16  --dashboard-port=8265  --dashboard-host=0.0.0.0
...
ray job submit --address="http://127.0.0.1:8265" \
  ...
  critic.forward_max_token_len_per_gpu=24000 \
  custom_reward_function.path=verl_utils/reward/reward_func.py \
  ...
  trainer.critique_prompt_idx=0 \
  trainer.n_gpus_per_node=8 \
  trainer.nnodes=1 \
  trainer.save_freq=8 \
  trainer.rollout_data_dir=/path/to/your/rollout_data_dir \
  trainer.test_freq=8 \
